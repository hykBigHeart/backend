...
import { LabelCreate } from "./compenents/create";
import { LabelUpdate } from "./compenents/update";
...
const ResourceCategoryPage = () => {
  ...
  const getData = () => {
    setLoading(true);
    labelManagement.resourceCategoryList().then((res: any) => {
      const labels: CategoriesBoxModel = res.data.labels;
      dispatch(saveCategoriesAction(res.data.labels));
  ...
  const checkArr = (labels: CategoriesBoxModel, id: number) => {
    const arr = [];
    for (let i = 0; i < labels[id].length; i++) {
      if (!labels[labels[id][i].id]) {
        arr.push({
          title: (
            <>
              <div className="tree-title-elli">{labels[id][i].name}</div>
              <div className="d-flex">
                ...
                <i
                  ...
                  style={{ fontSize: 24 }}
                  onClick={() =>
                    removeItem(labels[id][i].id, labels[id][i].name)
                  }
                />
              </div>
            </>
  ...
  const removeItem = (id: number, label: string) => {
    ...
    labelManagement.checkDestroy(id).then((res: any) => {
      if (
        res.data.children &&
  ...
  const delUser = (id: any) => {
    confirm({
      ...
      cancelText: "取消",
      onOk() {
        labelManagement.destroyResourceCategory(id).then((res: any) => {
  ...
  const submitDrop = (isTop: boolean, data: any, key: any) => {
    ...
    if (result) {
      if (isTop) {
        labelManagement.dropSameClass(result.ids).then((res: any) => {
          console.log("ok");
        });
      } else {
  ...
  const submitChildDrop = (key: any, pid: any, ids: any) => {
    labelManagement.dropDiffClass(key, pid, ids.ids).then((res: any) => {
      console.log("ok");
    });
  };
